    ...
    try:
      ...
      if b.info().get_content_maintype() == 'image':
        ...
        if source == 'saucenao' or source == 'sourcenao' or source == 'sourcenow':
          ...
          base64_str = base64.b64encode(buffered.getvalue()) # make it string (JSON can't serialise bytes)
        ...
        try:
          c = urllib.request.urlopen(query)
          await self.updateProgressBar(progressmsg, 60)
          d = c.read()
          if source == 'saucenao' or source == 'sourcenao' or source == 'sourcenow':
            urlresult = json.loads(d.decode('utf8'))
      ...
      await self.updateProgressBar(progressmsg, 100)
      return ReturnMessage('Couldn\'t access image. Error code: ' + str(e.code), None)
