import random
import difflib  # Import difflib for string difference calculation
from discord.ext import commands
from core import SakanyaCore

class Love():
  """
  This class provides functions for the command `>love`.
  """
  ...
  async def love(self, context, user=None):
    ...
    if context.message.server is None:
      await self.bot.say((
          'You can only send love to someone when you\'re in the same server as them.\n'
          'o(>< )o'))
      return
    ...
    if member['member'].bot is True:
      await self.bot.say((
          'Unfortunately, normal bots can\'t feel love... So I can\'t send the love to '
          'them. (-ω-、)'))
      return

    if member['member'].id == context.message.author.id:
      await self.bot.say((
          'Awww. Sending love to yourself... ⊂(･ω･*⊂) Well, loving yourself is always good!'))
    ...
    phrases = [
        ...
        ('Oh {0}, shall I compare thee to a summer\'s day? Thou art more lovely and more '
         'temperate. --{1}'),
        ...
        ('I dreamt of you last night {0}, and we were kissing and laughing. I woke up to realise '
         'that dream has already come true. --{1}')
    ]
    love_message = random.choice(phrases).format(member['member'].name, context.message.author.name)
    await self.bot.send_message(member['member'], (
        '❤ Here\'s a love message a special someone has sent you!\n'
        f'```{love_message}```'
    ))
